...
static void close(){
  ...
  static void selectMainMenu(String input){
...
//i에따라 메소드로 넘기기
    switch (input){
      case "1" : selectAddShopMenu();
      break;
      case "2" : selectDashboardMenu();
        break;
      case "3" : selectOrderMenu();
        break;
      case "4" : selectFeedbackMenu();
        break;
      default : System.out.println("[안내]메뉴에 포함된 번호를 입력하여 주세요.");
    }
  ...
  public static void selectAddShopMenu() {
    ...
    int price = s.nextInt();
    Shop shop = new Shop(shopName);//입력받은값 저장
    shop.addFood(foodName,price);
    shops.add(shop);//5번까지 받기
